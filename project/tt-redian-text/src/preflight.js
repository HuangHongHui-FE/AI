...
if (charCount(title) > 30) fail(`标题 ${charCount(title)} 字 >30：${title}`);
if (/(震惊|速看|刚刚|曝光)/.test(title)) fail(`标题党词：${title}`);

// === 摘要 ===
if (charCount(digest) > 100) fail(`digest ${charCount(digest)} 字 >100`);
