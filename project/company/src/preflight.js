...
// 公司介绍版：字数门放宽（1500-6000，长文讲清楚），其余硬门同 wechat-img
...
if (/据传|疑似|据说|有消息称|没法证实|有说法称|有人传/.test(body))
  fail('body 含存疑词（据传/疑似/据说等）——"引用存疑数据"低信息量铁证，须删或改可查证事实');
...
// 字数：公司介绍长文，1500-6000（不限死，讲清楚为主）
const bc = charCount(body);
if (bc < 1500) fail(`正文字数 ${bc} <1500，公司介绍须讲清楚，扩写`);
if (bc > 6000) fail(`正文字数 ${bc} >6000，过长，精简`);

// 可查证事实：扫数字/年份/金额，求职向 ≥6 处（薪资+营收双线，低信息量规避）
const factN = (body.match(/\d+(\.\d+)?%?|19\d{2}|20[0-2]\d|亿元|万亿|千万|百万|k\/月|K\/月|base|\d+[-－~]\d+\s*k/gi) || []).length;
if (factN < 6) fail(`可查证数字/事实 ${factN} 处 <6——求职向高信息量命脉，须补薪资区间/营收/市占率等数据`);
...
// === WARN（不阻断）===
if (boldN > 8) warn(`加粗 ${boldN} 处过多，全篇加粗等于没加粗`);
// 求职向核心：薪资区间数字（k/月、base、X-Yk）
const salaryN = (body.match(/\d+[-－~]\d+\s*k|base|薪|年终/gi) || []).length;
if (salaryN < 4) warn(`薪资相关词 ${salaryN} 处偏少——求职向薪资段是命脉，多列分档区间`);
...
console.log(`加粗 ${boldN} / 引言 ${quoteN} / 段落 ${paras.length} / 可查证数字 ${factN}`);
