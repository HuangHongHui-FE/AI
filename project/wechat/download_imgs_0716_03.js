// download_imgs_0716_03.js — 单独补搜第③篇配图（候选不足时用），扩展职场关键词
...
// 扩展更多关键词，增加候选量
const art = { dir: '03-公司让我整理那个刚被裁同事的工位',
  kws: ['办公室 空工位 椅子', '办公桌 抽屉 文件 旧', '写字楼 工位 离职', '办公室 窗外 黄昏 安静', '办公室 电脑 深夜', '茶水间 杯子 办公', '办公室 走廊 空旷', '工位 绿植 桌面'] };
...
async function main() {
  ...
  // 追加到 used_images.txt（不覆盖其他篇）
  const usedPath = path.join(BATCH_DIR, 'used_images.txt');
  let old = '';
  if (fs.existsSync(usedPath)) old = fs.readFileSync(usedPath, 'utf8');
