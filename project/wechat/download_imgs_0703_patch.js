// download_imgs_0703_patch.js — 给01/03补图（候选不足4张的两篇）
...
const arts = [
  { dir: '01-我把家里的钟都调慢了五分钟', kws: ['客厅 钟表 墙 挂钟', '家居 温馨 沙发 台灯', '家庭 客厅 夜晚'] },
  { dir: '03-35岁我把微信好友从800删到80', kws: ['手机 屏幕 深夜 床上', '微信 聊天 界面', '手机 相册 划'] },
];
...
async function main() {
  ...
  const existLines = fs.existsSync(usedFile) ? fs.readFileSync(usedFile,'utf8').trim().split('\n').filter(Boolean) : [];
  const newLines = [];
  for (const art of arts) {
    ...
    if (cands.length < 4) { console.log(`  ❌ 仍不足4张(${cands.length})`); continue; }
    ...
    newLines.push(`${art.dir}: ${pickedUrls.join(', ')}`);
  }
  fs.writeFileSync(usedFile, [...existLines, ...newLines].join('\n') + '\n');
  console.log('\n✅ 补图完成');
}
